...
describe('t2dm', () => {
	...
	describe('/POST create workflow', () => {
		...
		it('Add rx_t2dm_med-abnormal-lab (case rule 1) step.', async() => {
			stepId = await Workflow.upsertStep(workflowId, 3, "rx_t2dm_med-abnormal-lab", "An abnormal lab value is defined as one of three different exacerbations in blood sugar level.", "boolean");
		});
		...
		it('Add dx_t2dm-abnormal-lab (case rule 2) step.', async() => {
			stepId = await Workflow.upsertStep(workflowId, 4, "dx_t2dm-abnormal-lab", "A case is identified in the presence of an abnormal lab value (defined as one of three different exacerbations in blood sugar level) AND if a diagnosis of T2DM is identified.", "boolean");
		});
		...
		it('Add dx_t2dm-rx_t2dm_med (case rule 3) step.', async() => {
			stepId = await Workflow.upsertStep(workflowId, 5, "dx_t2dm-rx_t2dm_med", "Diagnosis of this type of diabetes AND a prescription of medication for it.", "boolean");
		});
		...
		it('Add dx_t2dm-rx_tdm_med-prec (case rule 4) step.', async() => {
			stepId = await Workflow.upsertStep(workflowId, 6, "dx_t2dm-rx_tdm_med-prec", "Diagnosis of this type of diabetes (by at least two physicians) AND a prescription of medication for both this type, and type 1, of diabetes AND type 1 prescribed prior.", "boolean");
		});
		...
		it('Add dx_t2dm-rx_t1dm_med-dx2 (case rule 5) step.', async() => {
			stepId = await Workflow.upsertStep(workflowId, 7, "dx_t2dm-rx_t1dm_med-dx2", "Diagnosis of this type of diabetes (by at least two physicians) AND a prescription of medication for type 1 of this type of diabetes.", "boolean");
		});
		...
		it('Add output-cases step.', async() => {
			stepId = await Workflow.upsertStep(workflowId, 8, "output-cases", "Output cases.", "output");
		});
