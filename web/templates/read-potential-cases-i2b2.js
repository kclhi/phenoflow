...
(async () => {
  ...
  await fs.appendFile('[PHENOTYPE]-potential-cases.csv', "patient-id,dob,codes\n");
  for(let patient in patients) {

    try {
      await fs.appendFile('[PHENOTYPE]-potential-cases.csv', patient+","+dobs[patient]+",\""+Array.from(patients[patient]).join(",")+"\"\n");
    } catch(error) {
      console.log(error);
    }
