...
3. Copy [visualiser/git-server/certs/pf.pem](visualiser/git-server/certs/pf.pem) to a new folder [visualiser/spring/certs](visualiser/spring/certs).
4. Copy [visualiser/git-server/certs/pf.pem](visualiser/git-server/certs/pf.pem) to a new folder [web/certs](web/certs).
5. Copy [visualiser/git-server/certs/pf.*](visualiser/git-server/certs) to [web/proxy/certs](web/proxy/certs).
...
Location of a root certificate within [web](web) (e.g. '[certs/pf.pem](web/certs/pf.pem)'), allowing 'web' to communicate with other services:

`NODE_EXTRA_CA_CERTS=`

Deployment RSA private key (for JWT signing) name, to be generated, e.g. `key.pem` generated by `openssl genrsa -out key.pem 2048`:

`RSA_PRIVATE_KEY=`

Path on target deployment host (without keyname), which may be local machine, e.g., `/home/ubunut/.certs` to deployment RSA private key:
...
Local development RSA private key full path, to reused from above, or newly generated and stored within the repo (e.g. as `certs/key.pem`):
...
Run specific test(s), e.g. to generate some initial phenotype definitions:

`NODE_ENV=test npx mocha -g "basic|covid|t2dm" --exit`
